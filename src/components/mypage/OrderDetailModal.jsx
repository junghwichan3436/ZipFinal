...
const OrderDetailModal = ({ isOpen, onClose, order, onCancelRequest }) => {
    ...
    <ModalOverlay>
      <ModalContainer>
        ...
        <ModalContent>
          ...
          <OrderDetailList>
            <li>
              주문상품: {order.productName} / {order.quantity}개 / KRW{" "}
              {order.price}
            </li>
          </OrderDetailList>
        </ModalContent>
        <ModalActions>
          {order.status !== "정상 배송" && (
            <ModalButton primary onClick={onCancelRequest}>
              {order.status === "교환 요청" ? "교환 취소" : "반품 취소"}
            </ModalButton>
          )}
        </ModalActions>
      </ModalContainer>
