    ...
    더보기
        ...
        <RelateProductsTitle>{findData?.artistName} Pick</RelateProductsTitle>
            ...
            >
                ...
                <SwiperSlide onClick={() => navigate(`/detail/${encodeURIComponent(product.itemName)}`)}>
                  {/* /detail/만 넣어줌으로써 localhost가 서로 이어붙지 않도록 해주었다
                      encodeURIComponent함수를 사용하면 20%를 자동으로 공백에 붙여준다*/}
                    <FilterItem>
                      <FilterItemImgWrap ><FilterItemImg src={product.detailImg.img01} alt="제품 이미지" /></FilterItemImgWrap>
                    <FilterItemText>
                      <FilterItemBrand >
                        {product.brand}
                        </FilterItemBrand>
          ...
          {matchedDataLists.map((matchedDataList) => (
              <SwiperSlide onClick={() => navigate(`/ott/detail/${encodeURIComponent(matchedDataList.snippet.title)}`)}>
                  <Thumbnails>
                    ...
                    <YouTube 
                      ...
                      videoId={matchedDataList.contentDetails.videoId}
                      opts={{
                        width: "100%",
                        height: "100%", // 반드시 이렇게! 300px 말고 100%
                        playerVars: {
                          autoplay: 0,
                          controls: 0,
                          modestbranding: 1,
                          rel: 0,
                          mute: 1, // 자동재생 위해 mute 필수
                          playsinline: 1,
                        },
                      }}
                    />
                  </StyledYouTubeWrapper> */}
                  <img src={matchedDataList?.snippet?.thumbnails?.standard?.url} alt="썸네일" />
                  </Thumbnails>
                <Profile>
                  <ProfileImg>
          ...
          {/* <img src={matchedDataLists[0]?.snippet?.thumbnails?.default?.url} alt="썸네일" /> */}
          </Swiper>
          <div></div>
        </VideoContents>
        </Container>
